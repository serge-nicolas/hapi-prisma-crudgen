...
const initPugAdmin = async (
  ...
  server.route({
    ...
    path,
    options: {
      handler: async (req, h: Hapi.ResponseToolkit) => {
        //LINK sample http://localhost:3002/admin/User/edit?where={"id":"6531287c1151a4838c2f6bd9"}
        let page: string = "";
        let itemData: any = {};
        logger.debug("route reached: " + req.path);
        logger.debug("route reached query: " + req.query.where);

        const parsedQuery: any = !!req.query && !!req.query.where ? JSON.parse(req.query.where) : null;
        ...
        const view = {
          ...
          action: currentRouteModel.action || null,// TODO deprecacte
